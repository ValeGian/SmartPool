...
static int sensed_chlorine_level = 2000;
...
pub_handler(const char *topic, uint16_t topic_len, const uint8_t *chunk,
            uint16_t chunk_len)
{
  if(strcmp(topic, "chlorine-actuator") == 0) {
    printf("Received Actuator command: ");

	if(strcmp((const char *)chunk, "ON")==0){
	        printf("Switch on chlorine regulator\n");
	        increase = true;
	    }else if (strcmp((const char *)chunk, "OFF")==0){
	        printf("Switch off chlorine regulator\n");
	        increase = false;
	    }else{
  ...
  while(1) {
    ...
    if((ev == PROCESS_EVENT_TIMER && data == &periodic_timer) ||
		  ...
		  if(state==STATE_CONNECTED){
			  ...
			  strcpy(sub_topic,"chlorine-actuator");
